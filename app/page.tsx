...
const fallbackDashboard: Dashboard = {
  momentum: { score: 82, delta: "+6.4% vs last week", summary: "Demo mode data. Sign in to sync your feed." },
  tier: { name: "Platinum", next: "Diamond", blockers: ["Weak-foot release under pace", "Cone exits dip after 4 reps"] },
  lastUpload: { title: "Dribble + wall pass set", reviewTime: "—", flags: ["Sign in for live flags"], boost: "+— momentum" },
  insights: [
    { label: "Current strength", detail: "Ball carry stays compact when accelerating.", tone: "success" },
    { label: "Micro-improvement", detail: "Plant 8–10cm wider on the weak foot to stay balanced.", tone: "muted" },
    { label: "Next session", detail: "Capture with AR ghost to verify cone spacing.", tone: "muted" },
  ],
  focus: [
    { title: "Dribbling control", delta: "+4%", description: "Hold sub-2s per gate and keep head checks every 2 touches." },
    { title: "Weak-foot accuracy", delta: "+5%", description: "Narrow release window; aim for knee height on exits." },
    { title: "Shooting power", delta: "+2%", description: "Push hip drive through the follow-through angle." },
  ],
  metrics: [
    { label: "Weak-foot usage", value: "42%", change: "+6%", intent: "up" },
    { label: "Dribble control", value: "86", change: "+8%", intent: "up" },
    { label: "Shooting accuracy", value: "77", change: "+4%", intent: "up" },
    { label: "Cone drill speed", value: "1.9s", change: "-0.06s", intent: "down" },
  ],
  weeklyCard: {
    ...
    <main className="main-wrap">
      <header className="hero-banner hero-layout">
        <div className="relative z-10 space-y-3">
          ...
          <h1 className="text-3xl md:text-4xl">Modern training OS for teenage footballers</h1>
        ...
        <div className="relative z-10 space-y-3">
          <div className="card card-strong">
            <div className="flex items-center justify-between">
              <div>
                <p className="label-muted">Momentum</p>
                <p className="text-4xl font-semibold leading-none">{dashboard.momentum.score}</p>
                <p className="text-success mt-1 text-sm">{dashboard.momentum.delta}</p>
              </div>
            ...
            <p className="text-secondary mt-2 text-sm">{dashboard.momentum.summary}</p>
          </div>
          <div className="card ghost-grid">
            <div className="flex items-center justify-between">
              <div>
            ...
            <div className="mt-3 grid grid-cols-2 gap-2 text-sm text-secondary">
              <span className="chip-muted">Stand here</span>
              <span className="chip-muted">Cone path</span>
              <span className="chip-muted">Ball rest</span>
              <span className="chip-muted">Ideal crop</span>
            </div>
          </div>
        </div>
      </header>
